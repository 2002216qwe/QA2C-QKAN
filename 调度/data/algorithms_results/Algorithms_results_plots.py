...
fig = px.scatter_3d(results_data, x='underconsume', y='overconsume', z='av_EV_energy_left',
                    ...
                    labels={
                     ...
                     "underconsume": "Unused RE-to-vehicle energy(kWh)",
                     "overconsume": "Grid energy used(kWh)",
                     "inverted_overconsume": "scaled-inverted Grid energy used(0-1)"
                    })
#fig.update_layout(scene_zaxis_type="log")
#ax.plot_surface(X, Y, Z, rstride=1, cstride=1, alpha=0.2)
...
fig.update_layout(title_text=f'Algorithms results scatter plot (size being the mapped Grid energy used [{round(min_overc/1000,1)}, {round(max_overc/1000,3)} MWh]-->[1,0])',  width=1600,height=800,)
...
fig = px.scatter(results_data, x='underconsume', y='av_EV_energy_left',
                    ...
                    labels={
                     ...
                     "underconsume": "Unused RE-to-vehicle energy(kWh)",
                     "overconsume": "Grid energy used(kWh)",
                     "inverted_overconsume": f"Grid energy used([{round(min_overc/1000,1)}, {round(max_overc/1000,3)} MWh]->[1,0])"
                    })

fig.update_layout(title_text=f'Algorithms results scatter plot (size being the mapped Grid energy used [{round(min_overc/1000,1)}, {round(max_overc/1000,3)} MWh]-->[1,0])',  width=1500,height=800,)
...
fig.add_trace(go.Scatter(x=x_plot, y=y_new, line={'color':'yellow'},
                            name="least Grid usage regression line"
                            ))
fig.update_layout(title_text='Algorithms results scatter plot (size being the scaled_cumulative_reward)',  width=1600,height=800,
                  coloraxis_colorbar=dict(yanchor="top", y=0.95, x=1,
                                          ticks="outside",
                                          #ticksuffix=" (kWh)"
                                          ))
